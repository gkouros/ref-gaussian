...
import math
from diff_surfel_rasterization_refgaussian import GaussianRasterizationSettings, GaussianRasterizer
...
def render_surfel(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, srgb = False, opt=None):
    ...
    diffuse_color = pc.get_diffuse_color
    ...
    contrib, rendered_image, rendered_features, radii, allmap = rasterizer(
        ...
        shs = shs,
        colors_precomp = colors_precomp,
        features = torch.cat((refl, roughness, ori_color, indirect, diffuse_color), dim=-1),
        opacities = opacity,
        scales = scales,
        rotations = rotations,
        cov3D_precomp = cov3D_precomp,
    ...
    refl_strength = rendered_features[0:1]
    roughness = rendered_features[1:2]
    albedo = rendered_features[2:5]
    indirect_light = rendered_features[5:8]
    base_color = rendered_features[8:11]  #TODO: DELETEME, use ori_color instead
    ...
    # diffuse = (1-refl_strength) * base_color  # original
    # diffuse = (1-refl_strength) * albedo * pc.get_envmap(normal_map, mode="diffuse").permute(2,0,1)  # TODO: DELETEME
    diffuse = (1-refl_strength) * base_color * pc.get_envmap(normal_map, mode="diffuse").permute(2,0,1)  # TODO: DELETEME
    ...
    if opt.indirect:
        indirect_color = diffuse + extra_dict['indirect_color']
